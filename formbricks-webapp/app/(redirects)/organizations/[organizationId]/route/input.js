...
// NAMESPACE OBJECT: ./app/(redirects)/organizations/[organizationId]/route.ts
...
;// ./app/(redirects)/organizations/[organizationId]/route.ts
...
const GET = async (_, context)=>{
    ...
    if (!organizationId) return (0,navigation_react_server.notFound)();
    // check auth
    ...
    const hasAccess = await (0,apiHelper/* hasOrganizationAccess */.s8)(session.user, organizationId);
    if (!hasAccess) throw new errors/* AuthorizationError */.yi("Unauthorized");
    const currentUserMembership = await (0,membership_service/* getMembershipByUserIdOrganizationId */.Y)(session?.user.id, organizationId);
    ...
    if (projects.length === 0) {
        return (0,navigation_react_server.redirect)(`/organizations/${organizationId}/landing`);
    }
    const firstProject = projects[0];
    const environments = await (0,service/* getEnvironments */.DL)(firstProject.id);
    const prodEnvironment = environments.find((e)=>e.type === "production");
    if (!prodEnvironment) return (0,navigation_react_server.notFound)();
    if (isBilling) {
        return (0,navigation_react_server.redirect)(`/environments/${prodEnvironment.id}/settings/billing`);
    }
    (0,navigation_react_server.redirect)(`/environments/${prodEnvironment.id}/`);
};

;// ../../node_modules/next/dist/build/webpack/loaders/next-app-loader/index.js?page=%2F(redirects)%2Forganizations%2F%5BorganizationId%5D%2Froute&name=app%2F(redirects)%2Forganizations%2F%5BorganizationId%5D%2Froute&pagePath=private-next-app-dir%2F(redirects)%2Forganizations%2F%5BorganizationId%5D%2Froute.ts&appDir=%2FUsers%2Fkdy1%2Fprojects%2Fformbricks%2Fapps%2Fweb%2Fapp&appPaths=%2F(redirects)%2Forganizations%2F%5BorganizationId%5D%2Froute&pageExtensions=tsx&pageExtensions=ts&pageExtensions=jsx&pageExtensions=js&basePath=&assetPrefix=&nextConfigOutput=standalone&nextConfigExperimentalUseEarlyImport=&preferredRegion=&middlewareConfig=e30%3D!
...
const routeModule = new module_compiled.AppRouteRouteModule({
    definition: {
        ...
        page: "/(redirects)/organizations/[organizationId]/route",
        pathname: "/organizations/[organizationId]",
        filename: "route",
        bundlePath: "app/(redirects)/organizations/[organizationId]/route"
    },
    resolvedPagePath: "/Users/kdy1/projects/formbricks/apps/web/app/(redirects)/organizations/[organizationId]/route.ts",
    nextConfigOutput,
...
var __webpack_exports__ = __webpack_require__.X(0, [8079,3073,7863,5069,255,9759,3687,4974], () => (__webpack_exec__(352446)));
